"""Kopf handlers for KeycloakClientRole resources."""
...
CLIENT_ROLE_CREATED_REASON = "ClientRoleCreated"
CLIENT_ROLE_DRIFT_DETECTED_REASON = "ClientRoleDriftDetected"
CLIENT_ROLE_MISSING_REASON = "ClientRoleMissing"
CLIENT_ROLE_OBSERVED_REASON = "ClientRoleObserved"
CLIENT_ROLE_ORPHANED_REASON = "ClientRoleOrphaned"
CLIENT_ROLE_UPDATED_REASON = "ClientRoleUpdated"
...
class KeycloakClientRoleClient(Protocol):
...
@dataclass(frozen=True)
class ClientRoleReconcileResult:
    ready_status: str
    ready_reason: str
    drift_detected: bool
...
def reconcile_keycloak_client_role(
    ...
    **_: Any,
    ...
    """Observe, create, or update a client role and patch status."""
    retry = patch_keycloak_client_role_status(
        spec=spec,
        status=status,
        patch=patch,
        namespace=namespace,
...
def delete_keycloak_client_role(
    ...
    **_: Any,
    ...
    """Delete the remote Keycloak client role when requested by policy."""
    deletion_policy = delete_keycloak_client_role_resource(spec=spec, namespace=namespace)
    _emit_delete_event(body, deletion_policy)


def delete_keycloak_client_role_resource(
    *,
    spec: Mapping[str, Any] | None,
    namespace: str | None = None,
    ...
    """Delete the remote Keycloak client role when deletionPolicy is Delete."""
    role_spec = _parse_client_role_spec(spec)
    ...
    try:
        ...
        raise _delete_temporary_error(
            "KeycloakClientRole deletion is waiting for the referenced KeycloakTarget."
        ) from None

    try:
        keycloak_client = keycloak_client_factory(**keycloak_client_factory_kwargs(target))
        keycloak_client.authenticate()
        delete_keycloak_client_role_if_exists(keycloak_client, role_spec)
        ...
        raise _delete_temporary_error(
            "KeycloakClientRole deletion failed while calling the Keycloak Admin API."
        ) from None


def patch_keycloak_client_role_status(
    *,
    spec: Mapping[str, Any] | None,
    status: Mapping[str, Any] | None,
    patch: MutableMapping[str, Any],
    ...
    """Patch KeycloakClientRole status after reconciliation."""
    ...
    if role_spec is None:
        ...
        _set_blocked_conditions(
            ...
            "Drift detection was skipped because the KeycloakClientRole spec is "
            "invalid.",
            now=now,
        )
        return None
    ...
    try:
        ...
        retry = RetryRequest(
            ...
            "KeycloakClientRole is not ready because the referenced KeycloakTarget "
    ...
    try:
        keycloak_client = keycloak_client_factory(**keycloak_client_factory_kwargs(target))
        keycloak_client.authenticate()
        reconcile_result = ensure_keycloak_client_role(keycloak_client, role_spec)
    except KeycloakAuthenticationError:
        retry = RetryRequest(
            AUTHENTICATION_FAILED_REASON,
            "KeycloakClientRole is not ready because Keycloak authentication failed.",
        )
        _set_blocked_conditions(
            patch,
            existing_conditions,
        ...
        retry = RetryRequest(
            ...
            "KeycloakClientRole reconciliation failed while calling the Keycloak "
            "Admin API.",
        )
        _set_blocked_conditions(
            patch,
    ...
    _set_conditions(
        ...
        existing_conditions,
        (
            _client_role_ready_condition(reconcile_result, now=now),
            _client_role_drift_condition(reconcile_result, now=now),
        ),
    )
    return None


def ensure_keycloak_client_role(
    client: KeycloakClientRoleClient,
    role_spec: ClientRoleSpec,
) -> ClientRoleReconcileResult:
    """Create, update, or observe a client role and return the result."""
    client_ref = _resolve_client_reference(client, role_spec)
    try:
        existing_role = client.request(
            "GET",
            _client_role_path(role_spec.realm, client_ref.internal_id, role_spec.name),
        )
    except KeycloakResourceNotFoundError:
        if role_spec.management_policy == MANAGEMENT_POLICY_OBSERVE_ONLY:
            return ClientRoleReconcileResult("False", CLIENT_ROLE_MISSING_REASON, True)

        client.request(
            "POST",
            _client_roles_path(role_spec.realm, client_ref.internal_id),
            json=_modeled_client_role_payload(role_spec),
        )
        created_role = client.request(
            "GET",
            _client_role_path(role_spec.realm, client_ref.internal_id, role_spec.name),
        )
        if not isinstance(created_role, Mapping):
            raise KeycloakRequestError(
                "Keycloak client role lookup response was not an object"
            ) from None
        return ClientRoleReconcileResult(
            "True",
            CLIENT_ROLE_CREATED_REASON,
            False,
            _remote_id(created_role),
        )
    ...
    if role_spec.management_policy == MANAGEMENT_POLICY_OBSERVE_ONLY:
        return ClientRoleReconcileResult(
            "True",
            CLIENT_ROLE_DRIFT_DETECTED_REASON,
            True,
            _remote_id(existing_role),
        )
    ...
    client.request(
        "PUT",
        _client_role_path(role_spec.realm, client_ref.internal_id, role_spec.name),
        json=_client_role_update_payload(existing_role, role_spec),
    )
    return ClientRoleReconcileResult(
        "True",
        CLIENT_ROLE_UPDATED_REASON,
        False,
        _remote_id(existing_role),
    )


def delete_keycloak_client_role_if_exists(
    client: KeycloakClientRoleClient,
    role_spec: ClientRoleSpec,
) -> None:
    """Delete an existing Keycloak client role or no-op when it is already missing."""
    client_ref = _find_client_reference(client, role_spec)
...
def _resolve_client_reference(
    ...
    client_ref = _find_client_reference(client, role_spec)
    if client_ref is None:
        raise KeycloakRequestError("Keycloak client role parent client was not found")

    return client_ref


def _find_client_reference(
    client: KeycloakClientRoleClient,
    role_spec: ClientRoleSpec,
) -> ClientReference | None:
    clients = client.request(
        "GET",
        _clients_path(role_spec.realm),
        params={"clientId": role_spec.client_id},
    )
    if not isinstance(clients, list):
        raise KeycloakRequestError("Keycloak client lookup response was not a list")

    parent = _matching_client(clients, role_spec.client_id)
    if parent is None:
        return None

    internal_id = parent.get("id")
    if not _is_non_empty_string(internal_id):
        raise KeycloakRequestError("Keycloak client role parent client did not include id")

    return ClientReference(internal_id.strip())


def _modeled_client_role_payload(role_spec: ClientRoleSpec) -> dict[str, Any]:
...
def _has_modeled_drift(
    existing_role: Mapping[str, Any],
    role_spec: ClientRoleSpec,
) -> bool:
    desired_payload = _modeled_client_role_payload(role_spec)
    return any(
...
def _client_role_update_payload(
    existing_role: Mapping[str, Any],
    role_spec: ClientRoleSpec,
) -> dict[str, Any]:
    payload = dict(existing_role)
    payload.update(_modeled_client_role_payload(role_spec))
    return payload
    ...
    *,
    ...
    if missing_fields:
        ...
        return ready_condition(
            "False",
            INVALID_SPEC_REASON,
            f"Missing required KeycloakClientRole spec fields: {fields}.",
            now=now,
        )
    ...
    if invalid_fields:
        return ready_condition(
            "False",
            INVALID_SPEC_REASON,
            invalid_spec_message("KeycloakClientRole", invalid_fields),
            now=now,
        )
    ...
    return ready_condition(
        "False",
        INVALID_SPEC_REASON,
        "KeycloakClientRole spec is invalid.",
        now=now,
    )
...
def _missing_required_fields(spec: Mapping[str, Any] | None) -> list[str]:
    ...
    if not _is_non_empty_string(client_name):
        missing_fields.append("clientRef.name")
...
def _client_role_ready_condition(
    reconcile_result: ClientRoleReconcileResult,
    *,
    now: datetime | None,
) -> Condition:
    if reconcile_result.ready_reason == CLIENT_ROLE_CREATED_REASON:
        message = "Keycloak client role was created."
    elif reconcile_result.ready_reason == CLIENT_ROLE_UPDATED_REASON:
        message = "Keycloak client role was updated."
    elif reconcile_result.ready_reason == CLIENT_ROLE_DRIFT_DETECTED_REASON:
        message = (
            "Keycloak client role has modeled drift and was not changed because "
            "managementPolicy is ObserveOnly."
        )
    elif reconcile_result.ready_reason == CLIENT_ROLE_MISSING_REASON:
        message = (
...
def _client_role_drift_condition(
    reconcile_result: ClientRoleReconcileResult,
    *,
    now: datetime | None,
) -> Condition:
    if not reconcile_result.drift_detected:
        return drift_detected_condition(
            ...
            "Keycloak client role has no modeled drift.",
            now=now,
        )

    if reconcile_result.ready_reason == CLIENT_ROLE_MISSING_REASON:
        message = (
            "Keycloak client role is missing and was not created because "
            "managementPolicy is ObserveOnly."
        )
        ...
        message = (
            "Keycloak client role differs from desired state and was not changed "
...
def _emit_reconcile_event(
    ...
    *,
    ...
    emit_event_for_condition_reasons(
        ...
        events={
            CLIENT_ROLE_CREATED_REASON: ("Normal", "Keycloak client role was created."),
            CLIENT_ROLE_UPDATED_REASON: ("Normal", "Keycloak client role was updated."),
            CLIENT_ROLE_DRIFT_DETECTED_REASON: (
                "Warning",
                "Keycloak client role has modeled drift and was left unchanged.",
            ),
            CLIENT_ROLE_MISSING_REASON: (
                "Warning",
                "Keycloak client role is missing and was left unchanged.",
            ),
        },
    )
...
def _emit_delete_event(body: kopf.Body, deletion_policy: str) -> None:
    if deletion_policy == DELETION_POLICY_DELETE:
        kopf.event(
            body,
            type="Normal",
            reason="ClientRoleDeleted",
            message="Keycloak client role was deleted because deletionPolicy is Delete.",
        )
        return
    ...
    kopf.event(
        body,
        type="Normal",
        reason=CLIENT_ROLE_ORPHANED_REASON,
        message="Keycloak client role was left in Keycloak because deletionPolicy is Orphan.",
    )
...
def _client_roles_path(realm: str, client_internal_id: str) -> str:
    return f"{_clients_path(realm)}/{quote(client_internal_id, safe='')}/roles"


def _client_role_path(realm: str, client_internal_id: str, role_name: str) -> str:
    return f"{_client_roles_path(realm, client_internal_id)}/{quote(role_name, safe='')}"
